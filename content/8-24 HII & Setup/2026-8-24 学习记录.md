...
BIOS Setup 是用户与固件配置之间唯一的交互界面，但它的本质不是"一堆界面代码"，而是一套**声明式表单 + 解释执行**的框架：表单用 VFR/SD 语言描述，编译成 IFR 二进制包注册进 HII Database；运行时由 Setup Browser（TSE）解析 IFR 渲染界面；用户每次按键修改，经 `EFI_HII_CONFIG_ACCESS_PROTOCOL` 转发到驱动侧处理。做过 MCU 的人可以把这套结构类比成"固件内置的微型 Web 服务"——VFR 相当于 HTML/DOM 声明，IFR 相当于编译后的字节码，Callback 相当于后端接口。本课基于 `Hii_Setup_v1.0.pptx`（27 页，edk2-stable202605 + AMI Aptio 5.x 环境）整理，覆盖 HII 架构、SD/UNI/C 三件套、三种 VarStore、问题类型与条件控制、ConfigAccess 回调机制，并用 edk2 `DriverSampleDxe` 源码做实战拆解。参考：edk2 VFR Spec 1.92、UNI Spec 1.40、UEFI Spec 2.10 §34.8/§35。

---

## 一、Setup 在 UEFI 中的地位：HII 与五类包
...
> [!tip]
> 从 MCU 视角看：HII Database 相当于"全局注册表 + 资源仓库"，Setup 驱动相当于"注册了自己配置服务的设备驱动"，Browser 相当于"读取注册表并渲染配置页的 UI 引擎"。三者的连接点是 FormSet GUID——驱动注册包和安装 ConfigAccess 时都用同一个 GUID，Browser 据此把"哪个页面"路由到"哪个驱动"。
...
PPT 附了一段真实 BIOS 串口 log，把 HII 初始化的完整时序串了起来：
...
## 十、DriverSampleDxe 实战拆解（edk2 源码）

源码位置：`MdeModulePkg/Universal/DriverSampleDxe/`（edk2 自带的 HII 参考驱动，2007 年起存在，专为演示 HII 而写）。
...
| # | 练习内容 | 涉及文件 | 预期结果 |
| --- | --- | --- | --- |
| 1 | 添加一个新的 oneof 问题 | Vfr.vfr + VfrStrings.uni + NVDataStruc.h | Setup 界面出现新选项 |
| 2 | 为新问题添加 fr-FR 翻译 | VfrStrings.uni | 切换语言显示法语 |
| 3 | 添加 suppressif 条件控制 | Vfr.vfr | 根据其他选项值隐藏/显示 |
| 4 | 创建新 Form 并用 goto 链接 | Vfr.vfr + VfrStrings.uni | 新页面可从主页面跳转 |
| 5 | 使用 efivarstore 存储到 NVRAM | Vfr.vfr + NVDataStruc.h | 重启后值保持不变 |
| 6 | 在 Callback 中处理新 QuestionId | DriverSample.c | 修改时触发自定义逻辑 |

建议顺序 1→6：先改 UI 声明，再深入存储与回调。

---

## 十一、与 BIOS 开发实践的衔接

- **HII 是 TSE 的底层**：AMI 的 TSE（Text Setup Environment，见 [[Hook_List]] 中 TSE Hook 机制）本质就是 AMITSE 这个 Browser 驱动的表现层。TSE Hook（HOOK35/AfterInitPostScreen 等）在 Browser 渲染流程的特定点插入 OEM 代码，Setup 页面的数据源则是本课讲的 HII 包。
- **Setup 变量就是 EFI 变量**：Setup 数据最终以 NV 变量（如 `SaSetup`、`CpuSetup`）存于 SPI Flash NVRAM，读写路径走 [[RunTime Services]] 的 GetVariable/SetVariable。第二章 log 中 `GetVariable 'SaSetup'` 即为动态字符串初始化读取配置。
- **ELINK 是 AMI 模块化的骨架**：SD/UNI/C 三件套都靠 ELINK 挂进 FormSet，详见 [[Aptio V eModule_oem & Elink]]。
- **调试手段**：UEFI Shell 下可用 `dmpstore` 查看/删除 Setup 变量（[[UEFI shell调试命令和APP]]），配合 `setup` 变量回滚测试默认值逻辑。
